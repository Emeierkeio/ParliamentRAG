...
Prompts used in the ISWC 2026 evaluation (experiments run in February 2026),
reproduced verbatim from the code of that period. The live system's prompts
may have evolved since; their current versions live in
...
| [Citation Introduction](#citation-introduction) | Support prompt — introductory text preceding a quotation |
...
| [Synthesis](#synthesis) | Cross-coalition convergence/divergence analysis |
...
All prompts are in Italian, the working language of the system.
...
Produces one section per parliamentary group from a structured position
brief, selecting the most incisive verbatim passage and enforcing the
quotation rules (single attribution, explicit stance, no reported speech).
...
Generates the 1–2 introductory sentences that precede a verbatim quotation,
so that the concatenation of introduction and quote is grammatically sound.
...
(Introduction, Government, Majority, Opposition), preserving all citation
markers and balancing coalition coverage.
...
Cross-coalition analysis appended to the report: convergences, fault lines,
and positions that cut across coalition boundaries.
...
baseline comparison. It asks for the same structure and behaviors that
ParliamentRAG enforces architecturally (per-group coverage, verbatim
quotations, balance), expressed as prompt-level guidance.
